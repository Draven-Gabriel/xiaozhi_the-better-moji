          ...
          <el-card class="device-card" shadow="never">
            ...
            >
              ...
              <template slot="deviceStatus" slot-scope="scope">
                <UiTag v-if="scope.row.deviceStatus === 'online'" type="success" size="small">{{ $t('device.online') }}</UiTag>
                <UiTag v-else type="default" size="small">{{ $t('device.offline') }}</UiTag>
              </template>
              <template slot="remark" slot-scope="scope">
                ...
                <span v-show="!scope.row.isEdit" class="remark-view">
                  <i class="el-icon-edit remark-edit-icon" @click="scope.row.isEdit = true"></i>
              ...
              <template slot="otaSwitch" slot-scope="scope">
                <el-switch v-model="scope.row.otaSwitch" size="mini" active-color="#0071e3" inactive-color="#DCDFE6"
                  @change="handleOtaSwitchChange(scope.row)"></el-switch>
              </template>
              <template slot="operations" slot-scope="scope">
                <UiButton type="text" size="small" class="unbind-btn" @click="handleUnbind(scope.row.device_id)">
                  {{ $t('device.unbind') }}
                </UiButton>
                <UiButton v-if="isGenerate(scope.row)" type="text" size="small" class="theme-btn" @click="handleGenertor(scope.row)">
...
export default {
  ...
  data() {
    return {
      ...
      agentSwitcherValue: this.$route.query.agentId || '',
      agentOptions: [],
      currentPage: 1,
      pageSize: 10,
  ...
  methods: {
    ...
    fetchAgentOptions() {
      Api.agent.getAgentList(({ data }) => {
        ...
        if (!targetId && list.length > 0) {
          targetId = list[0].id;
          this.$router.replace({ path: '/device-management', query: { agentId: targetId } });
        }
        this.agentSwitcherValue = targetId || '';
        if (targetId) {
          this.currentAgentId = targetId;
          this.fetchBindDevices(targetId);
        }
      });
    },
    handleAgentSwitch(agentId) {
      ...
      this.$router.replace({ path: '/device-management', query: { agentId } });
...
::v-deep .mac-address-mask {
  filter: blur(4px);
  transition: filter var(--apple-duration-fast) var(--apple-ease-standard);
}

::v-deep .mac-address-wrapper:hover .mac-address-mask {
  filter: none;
}
