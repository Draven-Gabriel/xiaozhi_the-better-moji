...
# ============================================================
# 无意义文本过滤（防止环境噪音/设备回声被误识别成对话，导致"抢话"）
# 覆盖中文/日文常见语气词、叹词的单字误识别
# ============================================================
MEANINGLESS_SINGLE_CHARS = set("嗯啊哦呃噢唔哎诶吖哈嘿咯唉哟喂哼切呸あうんえおん")


def is_meaningless_voice_text(text: str) -> bool:
    """判断 ASR 识别文本是否为无意义内容（噪音/语气词误识别）

    - 去除空白与标点后为空 → 无意义
    - 单个字符且属于语气词集合 → 无意义（环境噪音常被识别成"嗯/啊/あ"）
    - 仅由标点/emoji 组成 → 无意义
    """
    if not text:
        return True
    clean = "".join(ch for ch in text.strip() if ch.isalnum() or '\u4e00' <= ch <= '\u9fff' or '\u3040' <= ch <= '\u30ff')
    if not clean:
        return True
    if len(clean) == 1 and clean in MEANINGLESS_SINGLE_CHARS:
        return True
    return False
...
class ASRProviderBase(ABC):
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            # 过滤无意义文本（噪音/语气词误识别），避免"抢话"
            if text_len > 0 and not is_meaningless_voice_text(content_for_length_check):
