...
play_music_function_desc = {
    ...
    "function": {
        ...
        "description": (
            "当用户要求播放音乐时调用。规则："
            "1. 用户指定了歌名 → song_name 填歌名（如'七里香'）；"
            "2. 用户只说'放首歌/播放音乐/来点音乐'等、没有指定歌名 → song_name 必须填 'random'（随机播放）；"
            "3. 禁止反问用户'想听什么歌'，直接调用本工具。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "song_name": {
                    "type": "string",
                    "description": "歌曲名称，用户指定歌名时填歌名；未指定时固定填 'random'。示例：'播放两只老虎'→两只老虎；'播放音乐'→random",
                }
            },
            "required": ["song_name"],
        },
...
def _find_best_match(potential_song, music_files):
    """查找最匹配的歌曲（支持子目录：只用文件名匹配；支持包含匹配）"""
    best_match = None
    highest_ratio = 0

    for music_file in music_files:
        # 只取文件名（不含目录），修复子目录音乐匹配失败的问题
        song_name = os.path.splitext(os.path.basename(music_file))[0]
        # 包含匹配：'七里' 能命中 '七里香'
        if potential_song and (potential_song in song_name or song_name in potential_song):
            return music_file
