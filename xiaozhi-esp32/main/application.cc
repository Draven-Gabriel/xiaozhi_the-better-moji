    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_SEND_AUDIO) {
            if (GetDeviceState() == kDeviceStateSpeaking) {
                // 播放/说话期间不把麦克风实时传给服务器：
                // 仅在本地 VAD 确认人声后缓存，打断成功后再发给服务端。
                while (auto packet = audio_service_.PopPacketFromSendQueue()) {
#ifdef CONFIG_USE_DEVICE_AEC
                    if (bargein_capture_ && !bargein_triggered_) {
                        if (bargein_audio_cache_.size() >= 12) {  // 约 720ms
                            bargein_audio_cache_.pop_front();
                        }
                        bargein_audio_cache_.push_back(std::move(packet));
                    }
#else
                    (void)packet;
#endif
        ...
        if (bits & MAIN_EVENT_VAD_CHANGE) {
            if (GetDeviceState() == kDeviceStateListening) {
                last_vad_state_tick_ = clock_ticks_;
                if (vad_speaking_) {
                    last_vad_speech_tick_ = clock_ticks_;
                }
        ...
        if (bits & MAIN_EVENT_CLOCK_TICK) {
            ...
            // 本地自动停听：静音超过 5 秒主动结束聆听；
            // 如果 VAD 一直卡在“有声音”（残留回声/误判），超过 8 秒强制结束，
            // 避免等服务端超时太久。手动模式不自动停。
            if (GetDeviceState() == kDeviceStateListening &&
                listening_mode_ != kListeningModeManualStop &&
                ((!vad_speaking_ && (clock_ticks_ - last_vad_speech_tick_) >= 3) ||
                 (vad_speaking_ && (clock_ticks_ - last_vad_state_tick_) >= 5))) {
                    StopListening();
            }
...
void Application::HandleWakeWordDetectedEvent() {
    ...
    if (state == kDeviceStateIdle) {
        ...
        while (audio_service_.PopPacketFromSendQueue())
            ;
#ifdef CONFIG_USE_DEVICE_AEC
        bargein_capture_ = false;
        bargein_triggered_ = false;
        bargein_vad_start_ms_ = 0;
        bargein_audio_cache_.clear();
#endif
...
void Application::HandleStateChangedEvent() {
    ...
    clock_ticks_ = 0;
    last_vad_speech_tick_ = 0;
    last_vad_state_tick_ = 0;
    vad_speaking_ = false;
#ifdef CONFIG_USE_DEVICE_AEC
    bargein_capture_ = false;
    bargein_triggered_ = false;
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            display->SetEmotion("idle");     // Then set emotion
        ...
        case kDeviceStateSpeaking:
            ...
            display->SetEmotion("talking");

            // During playback/speaking: disable mic upload, VAD barge-in and wake word.
            // Without reliable AEC, the speaker would re-trigger itself endlessly.
            audio_service_.EnableVoiceProcessing(false);
...
void Application::AbortSpeaking(AbortReason reason) {
    ...
    aborted_ = true;
    // 本地立即停止当前播放/音乐，不用等服务端 tts:stop 往返。
    audio_service_.ResetDecoder();
    if (protocol_) {
...
#ifdef CONFIG_USE_DEVICE_AEC
void Application::CheckBargeIn() {
    if (GetDeviceState() != kDeviceStateSpeaking || aborted_ || bargein_triggered_ ||
        bargein_vad_start_ms_ == 0) {
        return;
    }
    const int64_t now_ms = esp_timer_get_time() / 1000;
    // 用户连续说话超过 1.2s 仍未停顿，也立即打断，避免等太久。
    if (now_ms - bargein_vad_start_ms_ >= 1200) {
        BargeIn(kAbortReasonNone);
    }
}

void Application::BargeIn(AbortReason reason) {
    if (GetDeviceState() != kDeviceStateSpeaking || bargein_triggered_) {
        return;
    }
    bargein_triggered_ = true;
    bargein_capture_ = false;
    ESP_LOGI(TAG, "Barge-in triggered (local VAD)");
    aborted_ = true;
    if (protocol_) {
        protocol_->SendAbortSpeaking(reason);
    }
...
void Application::FlushBargeInAudioCache() {
    while (!bargein_audio_cache_.empty()) {
        auto packet = std::move(bargein_audio_cache_.front());
        bargein_audio_cache_.pop_front();
        if (protocol_ && !protocol_->SendAudio(std::move(packet))) {
            break;
        }
...
#ifdef CONFIG_USE_DEVICE_AEC
    // 设备端 AEC + 本地打断：聆听时使用 realtime，
    // 但播放/说话期间麦克风由主循环拦截，不会误传给服务端。
