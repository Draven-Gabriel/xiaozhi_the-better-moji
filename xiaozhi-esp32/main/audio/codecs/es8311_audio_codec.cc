...
Es8311AudioCodec::Es8311AudioCodec(void* i2c_master_handle, i2c_port_t i2c_port, int input_sample_rate, int output_sample_rate,
    ...
    gpio_num_t pa_pin, uint8_t es8311_addr, bool use_mclk, bool pa_inverted, bool input_reference) {
    duplex_ = true; // 是否双工
    input_reference_ = input_reference; // 是否使用参考输入，实现回声消除
    input_channels_ = input_reference ? 2 : 1; // 输入通道数
    ...
    if (input_reference) {
        // 100ms @24kHz，足够覆盖喇叭到麦克风的传播延迟
        ref_buffer_.resize(2400 * 2);
    }
...
int Es8311AudioCodec::Read(int16_t* dest, int samples) {
    ...
    const int mic_samples = samples / 2;
    std::vector<int16_t> mic(mic_samples);
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_codec_dev_read(dev_, (void*)mic.data(), mic_samples * sizeof(int16_t)));

    int out = 0;
    {
        std::lock_guard<std::mutex> lock(ref_mutex_);
        // 如果扬声器已经停止输出超过 200ms，参考信号作废，
        // 避免 AEC 把用户打断后的第一句话也消掉。
        if (esp_timer_get_time() / 1000 - last_write_ms_ > 200) {
            read_pos_ = write_pos_ = 0;
        }
        for (int i = 0; i < mic_samples; ++i) {
            dest[out++] = mic[i];
            dest[out++] = (read_pos_ < write_pos_) ? ref_buffer_[read_pos_++] : 0;
        }
...
int Es8311AudioCodec::Write(const int16_t* data, int samples) {
    ...
    last_write_ms_ = esp_timer_get_time() / 1000;
    if (input_reference_ && samples > 0) {
        std::lock_guard<std::mutex> lock(ref_mutex_);
        const size_t written_size = static_cast<size_t>(samples);
        if (written_size >= ref_buffer_.size()) {
            memcpy(ref_buffer_.data(), data + (written_size - ref_buffer_.size()),
                   ref_buffer_.size() * sizeof(int16_t));
            read_pos_ = 0;
            write_pos_ = ref_buffer_.size();
        } else {
            if (write_pos_ - read_pos_ + written_size > ref_buffer_.size()) {
                read_pos_ += write_pos_ - read_pos_ + written_size - ref_buffer_.size();
            }
            if (read_pos_ > 0) {
            ...
            memcpy(&ref_buffer_[write_pos_], data, written_size * sizeof(int16_t));
            write_pos_ += written_size;
        }
    }
    return samples;
}
