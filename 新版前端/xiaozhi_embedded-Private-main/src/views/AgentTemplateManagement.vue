          ...
          <el-card class="template-card" shadow="never">
            ...
            >
              ...
              <template slot="operations" slot-scope="scope">
                <UiButton type="text" size="small" class="row-action-btn" @click="editTemplate(scope.row)">
                  {{ $t("agentTemplateManagement.editTemplate") }}
                </UiButton>
                <UiButton type="text" size="small" class="row-action-btn delete-btn" @click="deleteTemplate(scope.row)">
    ...
    fetchTemplateById(templateId) {
      agentApi.getAgentTemplateById(templateId, (res) => {
        if (res && res.data && res.data.code === 0 && res.data.data) {
          ...
          this.form = {
            id: template.id,
            agentCode: template.agentCode || "assistant",
