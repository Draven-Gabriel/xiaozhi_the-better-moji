            ...
            <div class="config-header">
              ...
              <div class="header-actions">
                <el-select
                  ...
                  :placeholder="form.agentName || $t('roleConfig.agentName')"
                  ...
                  <div class="form-column">
                    ...
                    <el-form-item class="context-provider-item">
                      ...
                      <div style="display: flex; align-items: center; justify-content: space-between;">
                        <span style="color: var(--apple-text-2); font-size: var(--apple-font-size-sm);">
...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve) => {
        Api.agent.getAgentList(({ data }) => {
          ...
          if (!targetId && list.length > 0) {
            targetId = list[0].id;
            this.$router.replace({ path: "/role-config", query: { agentId: targetId } });
          }
          this.agentSwitcherValue = targetId || "";
          if (targetId) {
            this.reloadAgentPage(targetId, { closeEditors: true });
          }
          resolve();
        });
      });
    ...
    handleAgentSwitch(agentId) {
      ...
      this.$router.replace({ path: "/role-config", query: { agentId } });
...
.current-version-tag {
  flex-shrink: 0;
  padding: 3px 10px;
  border: 1px solid var(--apple-brand-tint);
...
.header-icon img {
  filter: brightness(0) saturate(100%) invert(33%) sepia(92%) saturate(1814%) hue-rotate(199deg) brightness(96%) contrast(101%);
}
...
.header-actions .save-btn:not(.is-disabled):hover {
...
.header-actions .save-btn:not(.is-disabled):active {
  background: var(--apple-brand-active);
  border-color: var(--apple-brand-active);
  transform: translateY(0);
}

.header-actions .save-btn.is-disabled,
...
.header-actions .history-btn:hover {
  background: var(--apple-surface-2);
  border-color: var(--apple-brand-tint);
  color: var(--apple-brand);
  transform: translateY(-1px);
...
.header-actions .agent-switcher {
  width: 200px;
  margin-right: 4px;
  flex-shrink: 0;
}
.header-actions .agent-switcher ::v-deep .el-input__inner {
  border-radius: var(--apple-radius-md);
...
.header-actions .agent-switcher ::v-deep .el-input__inner:hover {
  border-color: $apple-brand;
}
.header-actions .agent-switcher ::v-deep .el-input.is-focus .el-input__inner {
  border-color: $apple-brand;
